...
- 🔎 **이름 검색**: 카테고리 필터와 함께 조합되는 실시간 텍스트 검색
- 📍 **내 위치 가까운순 정렬**: 브라우저 Geolocation API + Haversine 공식으로 거리 계산 및 정렬
- ♥ **찜하기**: localStorage 기반 즐겨찾기, "찜한 곳만 보기" 필터
- 🔧 **사장님 대시보드(`/admin`)**: 전체 매장 KPI(대기중 매장 수, 전체 대기 팀 수)와 매장별 웨이팅 일괄 관리
...
- **거리순 정렬을 클라이언트에서 계산한 이유**: 지금은 음식점이 18개뿐이라 클라이언트에서 Haversine 공식으로 거리를 계산해도 충분하지만, 데이터가 많아지면 이 계산을 매번 클라이언트가 모든 항목에 대해 반복하는 건 비효율적입니다. 실제 서비스라면 DB 레벨의 공간 인덱스(PostGIS의 `ST_Distance` 등)로 "반경 N km 이내"를 먼저 필터링한 뒤, 그 결과만 정렬하는 방식으로 가야 합니다.
- **찜하기를 서버 DB가 아닌 localStorage로 구현한 이유**: 로그인/인증 기능이 없는 토이 프로젝트 범위에서, 사용자를 식별할 방법이 없어 서버에 저장해도 누구의 찜인지 구분할 수 없습니다. 실제 서비스라면 로그인 사용자 기준으로 DB에 저장해야 합니다.
